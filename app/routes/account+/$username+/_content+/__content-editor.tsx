...
import { cn, getContentImgSrc, useDebounce, useIsPending } from '#app/utils/misc.tsx'
...
export function ContentEditor({
  ...
  function isActionDataWithResult(
    ...
    return data != null && 'result' in data
  }

  const [form, fields] = useForm({
    id: 'content-editor',
    constraint: getZodConstraint(ContentEditorSchema),
    lastResult: isActionDataWithResult(actionData) ? actionData.result : undefined,
    onValidate({ formData }) {
  ...
  const debouncedAutoSave = useDebounce(handleAutoSave, 1000)
    ...
    <FormProvider context={form.context}>
      <Form
        ...
        onSubmit={(event) => {
          ...
          submit(event.currentTarget, { method: 'POST', encType: 'multipart/form-data' })
        }}
      ...
      >
        ...
        <div className="flex flex-col gap-1">
          <div>
            ...
            <TiptapEditorWrapper field={fields.title} />
            <ErrorList id={fields.title.errorId} errors={fields.title.errors} />
          </div>
          <div>
            <Label htmlFor={fields.content.id}>Content</Label>
            <TiptapEditorWrapper field={fields.content} />
...
function TiptapEditorWrapper({ field }: { field: FieldMetadata<string> }) {
  const control = useInputControl(field)
  return (
    <TiptapEditor
      content={control.value ?? ''}
      onChange={(newContent) => {
        control.change(newContent)
      }}
    />
  )
}
